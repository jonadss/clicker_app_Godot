...
func _process(delta: float) -> void:
	zahl_label.text = str(global.clicker_stand_02)
...
func apply_swipe(end_pos: Vector2):
	...
	if abs_x > abs_y and abs_x > swipe_threshold:
		...
		if delta_steps != 0:
			global.clicker_stand_02 += delta_steps * step_size  # umdrehen für natürliches Wischen
	start_pos = Vector2.ZERO
	delta_steps = 0

# ----- Szenewechsel-Logik -----
func on_swipe_left():
	print("geht nicht weiter")  # z.B. nächste Szene
...
func _on_top_pressed() -> void:
	global.clicker_stand_02 += step_size
